...
var admin_calendar_exports = {};
__export(admin_calendar_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(admin_calendar_exports);
...
async function handler(event) {
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      const bookingsQ = supabase.from("bookings").select("id, customer_name, pack, start_time, end_time, status").eq("status", "reserved").lt("start_time", to).gt("end_time", from).order("start_time", { ascending: true });
      const blackoutsQ = supabase.from("blackouts").select("id, reason, start_time, end_time").lt("start_time", to).gt("end_time", from).order("start_time", { ascending: true });
      const [
    ...
    if (event.httpMethod === "POST") {
      ...
      const { data, error } = await supabase.from("blackouts").insert(rows).select("id, reason, start_time, end_time");
