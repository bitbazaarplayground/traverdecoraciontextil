...
// node_modules/@supabase/realtime-js/node_modules/tslib/tslib.es6.mjs
...
var extendStatics2, __assign2, __createBinding2, __setModuleDefault2, ownKeys2, _SuppressedError2, tslib_es6_default2;
...
__export(tslib_es6_exports3, {
  ...
  __read: () => __read3,
  __rest: () => __rest3,
  __rewriteRelativeImportExtension: () => __rewriteRelativeImportExtension3,
...
function __importStar3(mod) {
  ...
  var result = {};
  if (mod != null) {
    for (var k = ownKeys4(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding3(result, mod, k[i]);
  }
...
function __rewriteRelativeImportExtension3(path, preserveJsx) {
  if (typeof path === "string" && /^\.\.?\//.test(path)) {
    return path.replace(/\.(tsx)$|((?:\.d)?)((?:\.[^./]+?)?)\.([cm]?)ts$/i, function(m, tsx, d, ext, cm) {
      return tsx ? preserveJsx ? ".jsx" : ".js" : d && (!ext || !cm) ? m : d + ext + "." + cm.toLowerCase() + "js";
    });
...
var extendStatics3, __assign3, __createBinding3, __setModuleDefault3, ownKeys4, _SuppressedError3, tslib_es6_default3;
...
// netlify/functions/admin-customer-images.js
var admin_customer_images_exports = {};
__export(admin_customer_images_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(admin_customer_images_exports);
...
function _objectSpread22(e) {
  for (var r = 1; r < arguments.length; r++) {
    var t = null != arguments[r] ? arguments[r] : {};
    r % 2 ? ownKeys5(Object(t), true).forEach(function(r$1) {
      _defineProperty2(e, r$1, t[r$1]);
    }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : ownKeys5(Object(t)).forEach(function(r$1) {
      Object.defineProperty(e, r$1, Object.getOwnPropertyDescriptor(t, r$1));
    });
  }
  return e;
...
function safeFilename(name) {
  return String(name || "image").trim().replace(/[^\w.\-]+/g, "_");
}
async function handler(event) {
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      const { data: rows, error: rowsErr } = await supabase.from("customer_images").select(
        "id, customer_key, path, filename, content_type, caption, created_at"
      ).eq("customer_key", key).order("created_at", { ascending: false }).limit(200);
      if (rowsErr) return json(500, { error: rowsErr.message });
      const paths = (rows || []).map((r) => r.path).filter(Boolean);
      const signed = await Promise.all(
        paths.map(async (p) => {
          const { data, error: signErr } = await supabase.storage.from(bucket).createSignedUrl(p, 60 * 60);
    ...
    if (event.httpMethod === "POST") {
      ...
      const { error: uploadErr } = await supabase.storage.from(bucket).upload(path, buffer, { contentType, upsert: false });
      if (uploadErr) return json(500, { error: uploadErr.message });
      const { data: inserted, error: insErr } = await supabase.from("customer_images").insert({
        customer_key: key,
        path,
        filename,
    ...
    if (event.httpMethod === "PATCH") {
      ...
      const { data: updated, error: updErr } = await supabase.from("customer_images").update({ caption }).eq("id", id).select(
        "id, customer_key, path, filename, content_type, caption, created_at"
      ).single();
      if (updErr) return json(500, { error: updErr.message });
      const { data: signed, error: signErr } = await supabase.storage.from(bucket).createSignedUrl(updated.path, 60 * 60);
      return json(200, {
    ...
    if (event.httpMethod === "DELETE") {
      const id = body?.id;
      if (!id) return json(400, { error: "Missing id" });
      const { data: row, error: rowErr } = await supabase.from("customer_images").select("id, path").eq("id", id).single();
      ...
      const { error: delErr } = await supabase.from("customer_images").delete().eq("id", id);
...
//# sourceMappingURL=admin-customer-images.js.map
