...
var enquiry_exports = {};
__export(enquiry_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(enquiry_exports);
...
async function handler(event) {
  try {
    ...
    const cleanMeetingMode = String(meeting_mode || "").trim().toLowerCase();
    ...
    const inserted = await supabaseInsert({
      ...
      row: {
        ...
        customer_name: name,
        phone: phoneNorm,
        // store normalized digits or null
        email: emailLower,
        // store lowercase or null
